...
public class BookInventoryMockImpl implements BookInventory 
{
	...
	/*
	 * In order to have a listing of categories, without having to go through the whole map every time,
	 * we keep a count of the books that are in a category and update that count as we store end remove books
	 */
	...
	{
		Book book = this.booksByISBN.get(isbn);
		if(book == null)
		{
			throw new BookNotFoundException(isbn);
		}
	...
	private boolean checkIntegerGreater(int attr, String ctriVal)
	{
		int critVal;
		try
		{
			critVal = Integer.parseInt(ctriVal);
		}
	...
	private boolean checkIntegerSmaller(int attr, String ctriVal)
	{
		int critVal;
		try
		{
